...
namespace avmplus {
	namespace NativeID {
		using namespace avmthane;
		#include "thanelib.cpp"
	}
...
namespace avmthane
{
	...
	{
		shellClasses = (ClassClosure**) core()->GetGC()->Calloc(avmplus::NativeID::thanelib_abc_class_count, sizeof(ClassClosure*), MMgc::GC::kZero | MMgc::GC::kContainsPointers);
	}

	void Shell::usage()
	{
		AvmLog("avmplus shell " AVMPLUS_VERSION_USER " build " AVMPLUS_BUILD_CODE "\n\n");
		AvmLog("usage: avmthane\n");
		#ifdef DEBUGGER
	...
	{
		shellPool = AVM_INIT_BUILTIN_ABC(thanelib, this);
	}
	...
	int Shell::execute(int argc, char *argv[])
	{
		...
		TRY(this, kCatchAction_ReportAsError)
		{
            ...
            // this is basically Toplevel::findClassInPool, which is protected for some reason
            Traits* traits = shellPool->getClassTraits(avmplus::NativeID::abcclass_Thane)->itraits;
            Multiname qname(traits->ns, traits->name);
            ScriptObject* thaneObj = toplevel->global()->vtable->init->finddef(&qname);
            ...
            Stringp heartName = internString(newConstantStringLatin1("heartbeat"));
            Binding heartBinding = cc->traits()->getTraitsBindings()->findBinding(heartName);
            MethodEnv *heartMethod = cc->vtable->methods[AvmCore::bindingToMethodId(heartBinding)];
